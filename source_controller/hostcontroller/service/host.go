...
import (
	...
	"github.com/tencent/bk-cmdb/common/eventclient"
...
func (s *Service) DelModuleHostConfig(req *restful.Request, resp *restful.Response) {
	...
	if 0 == len(params.ModuleID) {
		var err error
		moduleIDs, err = s.Logics.GetModuleIDsByHostID(ctx, getModuleParams) //params.HostID, params.ApplicationID)
		if nil != err {
...
func (s *Service) DelDefaultModuleHostConfig(req *restful.Request, resp *restful.Response) {
	...
	defaultModuleIDs, err := s.Logics.GetDefaultModuleIDs(ctx, params.ApplicationID)
	if nil != err {
...
func (s *Service) AssignHostToApp(req *restful.Request, resp *restful.Response) {
	...
	for _, hostID := range params.HostID {
		...
		moduleIDs, err := s.Logics.GetModuleIDsByHostID(ctx, getModuleParams)
		if nil != err {
