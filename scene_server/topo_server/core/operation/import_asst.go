...
import (
	...
	sceneUtil "github.com/tencent/bk-cmdb/scene_server/common/util"
)
...
func (a *asstObjectInst) getObjectFields(objID, sort string) ([]metadata.ObjAttDes, error) {
	...
	cond.Field(metadata.AttributeFieldObjectID).Eq(objID)
	cond.Field(metadata.AttributeFieldSupplierAccount).Eq(a.ownerID)
	cond.SetLimit(common.BKNoLimit)
	cond.SetSort(common.BKPropertyIDField)

	result, err := a.CoreAPI.ObjectController().Meta().SelectObjectAttWithParams(context.Background(), a.pheader, cond.ToMapStr())
	if err != nil || (err == nil && !result.Result) {
