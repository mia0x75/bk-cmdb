...
func (cli *eventHostIdentifier) constructEvent(currHost, preHost types.MapStr) (types.MapStr, error) {
	return nil, nil
}
func (cli *eventHostIdentifier) parse(data types.MapStr) ([]*types.Event, error) {
	...
	for _, dataItem := range dataArr {
		...
		preHost, err := dataItem.MapStr("pre_data")
		...
		ev := &types.Event{}
		ev.SetCurrData(curHost)
		ev.SetPreData(preHost)
		ev.SetAction(action)
		ev.SetActionTime(*tm)
		eves = append(eves, ev)
