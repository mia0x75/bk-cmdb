...
import (
	...
	"github.com/tencent/bk-cmdb/framework/core/output"
...
func (cli *eventSet) constructEvent(curSet, preSet types.MapStr) (types.MapStr, error) {
	return nil, nil
}
func (cli *eventSet) parse(data types.MapStr) ([]*types.Event, error) {
	...
	for _, dataItem := range dataArr {
		...
		curSet, err := dataItem.MapStr("cur_data")
		...
		preSet, err := dataItem.MapStr("pre_data")
		...
		ev := &types.Event{}
		ev.SetCurrData(curSet)
		ev.SetPreData(preSet)
		ev.SetAction(action)
		ev.SetActionTime(*tm)
		eves = append(eves, ev)
